...
/* ========== Angle Tracking ========== */
static float s_current_angle = 0.0f;  /**< Current angle relative to home position (0 degrees) */

/* ========== Watchdog Feed Configuration ========== */
#define WDT_YIELD_INTERVAL_STEPS  200  /**< Yield CPU every N steps to prevent watchdog timeout */
...
static void stepper_rotate_cw_with_accel(int steps, int target_delay_us)
{
    ...
    int actual_steps = 0;  // Track actually executed steps
    
    ESP_LOGI(TAG, "CW profile: total=%d, accel=%d, const=%d, decel=%d, start=%dus->target=%dus", 
             steps, accel_steps, constant_steps, decel_steps, start_delay_us, target_delay_us);
    ...
    for (int i = 0; i < accel_steps; i++) {
        ...
        actual_steps++;
        // Yield CPU periodically to prevent watchdog timeout during long rotations
        if ((i % WDT_YIELD_INTERVAL_STEPS) == 0) {
            taskYIELD();
        }
    }
    ...
    ESP_LOGI(TAG, "CW done: executed %d/%d steps", actual_steps, steps);
}
...
static void stepper_rotate_ccw_with_accel(int steps, int target_delay_us)
{
    ...
    ESP_LOGI(TAG, "CCW profile: total=%d, accel=%d, const=%d, decel=%d, start=%dus->target=%dus", 
             steps, accel_steps, constant_steps, decel_steps, start_delay_us, target_delay_us);
    ...
    ESP_LOGI(TAG, "CCW done: executed %d/%d steps", actual_steps, steps);
}
...
void stepper_rotate_angle_with_accel(float angle, int target_delay_us)
{
    ...
    if (steps == 0) {
        ESP_LOGI(TAG, "Angle too small (%.2f°), no rotation needed", angle);
        return;
    }
    
    // Log actual rotation for debugging
    ESP_LOGI(TAG, "Rotating: angle=%.1f°, steps=%d, speed=%dus/step", 
             angle, steps > 0 ? steps : -steps, target_delay_us);
    ...
    // Update current angle tracking (no clamping to maintain accuracy)
    s_current_angle += angle;
    
    ESP_LOGI(TAG, "Rotation complete, current angle: %.1f°", s_current_angle);
}
...
void stepper_shake_head(float amplitude, int cycles, int speed_us)
{
    ...
    // Calculate expected total rotation for verification
    float expected_total = amplitude + (cycles * 4 * amplitude) + amplitude;
    int expected_steps = (int)(expected_total * 4128.0 / 360.0);
    
    ESP_LOGI(TAG, "=== SHAKE_HEAD START === amplitude=%.1f°, cycles=%d, speed=%dus", 
             amplitude, cycles, speed_us);
    ESP_LOGI(TAG, "Expected total rotation: %.1f° (%d steps)", expected_total, expected_steps);
    
    float start_angle = s_current_angle;
    
    // Step 1: Turn left from center to start position
    ESP_LOGI(TAG, "[1/3] Move to left start: -%.1f°", amplitude);
    stepper_rotate_angle_with_accel(-amplitude, speed_us);
    vTaskDelay(pdMS_TO_TICKS(10));  // Brief pause
    
    // Shake cycles
    for (int i = 0; i < cycles; i++) {
        ESP_LOGI(TAG, "[2/3] Shake cycle %d/%d: right +%.1f° then left -%.1f°", 
                 i + 1, cycles, 2 * amplitude, 2 * amplitude);
    ...
    ESP_LOGI(TAG, "[3/3] Return to center: +%.1f°", amplitude);
    stepper_rotate_angle_with_accel(amplitude, speed_us);
    
    float end_angle = s_current_angle;
    ESP_LOGI(TAG, "=== SHAKE_HEAD END === start=%.1f°, end=%.1f°, drift=%.1f°", 
...
/**
 * @brief Get current motor angle relative to home position
 * 
 * @return float Current angle in degrees (0 = home position)
 */
float stepper_get_current_angle(void)
{
    return s_current_angle;
}

/**
 * @brief Reset current angle to zero (call after calibration)
 */
void stepper_reset_angle(void)
{
    s_current_angle = 0.0f;
    ESP_LOGI(TAG, "Angle reset to 0 (home position)");
}
