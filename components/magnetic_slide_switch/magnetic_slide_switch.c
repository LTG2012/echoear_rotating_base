...
static bool s_request_recalibration = false;  /**< Flag to request recalibration */
#endif
...
static BMM150_INTF_RET_TYPE bmm150_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    if (s_i2c_dev == NULL) {
        ...
        if (!s_read_error_logged) {
            ESP_LOGE(TAG, "I2C device not initialized (read)");
...
static BMM150_INTF_RET_TYPE bmm150_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    // Check if I2C device handle is valid
    if (s_i2c_dev == NULL) {
        static bool s_write_error_logged = false;
        if (!s_write_error_logged) {
            ESP_LOGE(TAG, "I2C device not initialized (write)");
            s_write_error_logged = true;
        }
        return BMM150_E_COM_FAIL;
    }
            ...
            if (s_calibration_state != CALIBRATION_COMPLETED) {
                ...
                switch (s_calibration_state) {
                            ...
                            if (diff < 10) {  // Value is stable (within 10 units)
                                ...
                                if (abs(average - s_stable_value) < 10) {
                                    ...
                                    if (stable_duration >= stability_ticks) {
                                        ...
                                        // Send notification: first position done, prompt user to move to second position
                    ...
                    case CALIBRATION_WAITING_CHANGE_1:
                    {
                        ...
                        // 调试日志:周期性输出等待第二个位置变化的状态
                        static uint8_t s_wait1_counter = 0;
                        s_wait1_counter++;
                        if (s_wait1_counter >= 50) {  // 每50次采样(约500ms)输出一次
                            s_wait1_counter = 0;
                            ESP_LOGI(TAG, "[WAIT1] 等待第二个位置... 当前值=%d, 第一位置=%d, 差异=%d, 需要差异>%d",
                                     average, s_calibration_temp_values[0], diff_from_first,
                                     CALIBRATION_VALUE_DIFF_THRESHOLD);
                        }
                        
                        if (diff_from_first > CALIBRATION_VALUE_DIFF_THRESHOLD) {
                            // Value changed significantly, enter second position detection
                            ESP_LOGI(TAG, "检测到第一个位置的变化 (差异=%d), 开始检测第二个位置...", diff_from_first);
                    ...
                    case CALIBRATION_DETECTING_SECOND:
                    {
                        ...
                        if (diff_from_first <= CALIBRATION_VALUE_DIFF_THRESHOLD) {
                            ...
                            // 调试日志:值太接近第一个位置
                            static uint8_t s_second_reject_counter = 0;
                            s_second_reject_counter++;
                            if (s_second_reject_counter >= 20) {
                                s_second_reject_counter = 0;
                                ESP_LOGW(TAG, "[SECOND] 值太接近第一位置被拒绝: 当前=%d, 第一位置=%d, 差异=%d (需要>%d)",
                                         average, s_calibration_temp_values[0], diff_from_first,
                                         CALIBRATION_VALUE_DIFF_THRESHOLD);
                            }
                            s_calibration_last_average = 0;
                        ...
                        // 调试日志:值满足差值要求,检查稳定性
                        static uint8_t s_second_accept_counter = 0;
                        s_second_accept_counter++;
                        if (s_second_accept_counter >= 20) {
                            s_second_accept_counter = 0;
                            ESP_LOGI(TAG, "[SECOND] 值可接受: 当前=%d, 差异=%d, 检查稳定性...",
                                     average, diff_from_first);
                        }
                        
                        // Check if value is stable
                        if (s_calibration_last_average == 0) {
                            // First reading, initialize
                            ESP_LOGI(TAG, "[SECOND] 开始稳定性跟踪,初始值=%d", average);
                            s_calibration_last_average = average;
                            s_stable_start_time = xTaskGetTickCount();
                            s_stable_value = average;
                        } else {
                            ...
                            // 调试日志:输出稳定检测进度
                            TickType_t current_time = xTaskGetTickCount();
                            TickType_t stable_duration = (s_stable_start_time > 0) ? 
                                                         (current_time - s_stable_start_time) : 0;
                            static uint8_t s_second_stability_counter = 0;
                            s_second_stability_counter++;
                            if (s_second_stability_counter >= 20) {
                                s_second_stability_counter = 0;
                                ESP_LOGI(TAG, "[SECOND] 稳定性检查: 当前=%d, 稳定值=%d, 波动=%d, 已稳定=%lums, 需要=%dms",
                                         average, s_stable_value, diff,
                                         (unsigned long)(stable_duration * portTICK_PERIOD_MS),
                                         CALIBRATION_STABILITY_TIME_MS);
                            }
                            ...
                            if (diff < 10) {  // Value is stable (within 10 units)
                                ...
                                if (abs(average - s_stable_value) < 10) {
                                    ...
                                    if (stable_duration >= stability_ticks) {
                                        ...
                                        ESP_LOGI(TAG, "第二个位置已校准: %d (稳定 %lu ms, 与第一位置差异=%d)", 
                                                 average, (unsigned long)(stable_duration * portTICK_PERIOD_MS), diff_from_first);
                                        
                                        // Send notification: second position done, prompt user to remove slider
                                        control_serial_send_magnetic_switch_calibration_step(MAG_SWITCH_CALIB_SECOND_DONE);
                                    ...
                                    // Value changed but still stable, update stable value and time
                                    ESP_LOGW(TAG, "[SECOND] 稳定值漂移: 当前=%d, 之前=%d, 重置稳定计时器",
                                             average, s_stable_value);
                                    s_stable_value = average;
                                    s_stable_start_time = xTaskGetTickCount();
                                }
                            } else {
                                // Value changed significantly, reset stable tracking
                                ESP_LOGW(TAG, "[SECOND] 值不稳定(波动=%d >= 10), 重置: 当前=%d, 上次=%d",
                                         diff, average, s_calibration_last_average);
                                s_stable_value = average;
                                s_stable_start_time = xTaskGetTickCount();
                            }
                    ...
                    case CALIBRATION_WAITING_CHANGE_2:
                    {
                        ...
                        // 调试日志：输出等待第三个位置变化的状态
                        static uint8_t s_wait2_counter = 0;
                        s_wait2_counter++;
                        if (s_wait2_counter >= 50) {
                            s_wait2_counter = 0;
                            ESP_LOGI(TAG, "[WAIT2] avg=%d, pos1=%d (diff=%d), pos2=%d (diff=%d), need diff>%d",
                                     average, s_calibration_temp_values[0], diff_from_first,
                                     s_calibration_temp_values[1], diff_from_second,
                                     CALIBRATION_VALUE_DIFF_THRESHOLD);
                        }
                        ...
                        if (diff_from_second > CALIBRATION_VALUE_DIFF_THRESHOLD) {
                            ...
                            ESP_LOGI(TAG, "[WAIT2->THIRD] Entering CALIBRATION_DETECTING_THIRD, pos1=%d, pos2=%d",
                                     s_calibration_temp_values[0], s_calibration_temp_values[1]);
                    ...
                    case CALIBRATION_DETECTING_THIRD:
                    {
                        ...
                        // 调试日志：每10次采样输出一次当前状态
                        static uint8_t s_third_debug_counter = 0;
                        s_third_debug_counter++;
                        if (s_third_debug_counter >= 10) {
                            s_third_debug_counter = 0;
                            ESP_LOGD(TAG, "[THIRD] avg=%d, pos1=%d, pos2=%d, diff1=%d, diff2=%d, threshold=%d",
                                     average, s_calibration_temp_values[0], s_calibration_temp_values[1],
                                     diff_from_first, diff_from_second, CALIBRATION_VALUE_DIFF_THRESHOLD);
                        }
                        ...
                        if (diff_from_first <= CALIBRATION_VALUE_DIFF_THRESHOLD || 
                            ...
                            // 调试日志：输出被拒绝的原因
                            static uint8_t s_reject_counter = 0;
                            s_reject_counter++;
                            if (s_reject_counter >= 20) {
                                s_reject_counter = 0;
                                ESP_LOGW(TAG, "[THIRD] Rejected: avg=%d, diff1=%d (need >%d), diff2=%d (need >%d)",
                                         average, diff_from_first, CALIBRATION_VALUE_DIFF_THRESHOLD,
                                         diff_from_second, CALIBRATION_VALUE_DIFF_THRESHOLD);
                            }
                            s_calibration_last_average = 0;
                            s_stable_start_time = 0;
                        ...
                        // 调试日志：值满足差值要求
                        ESP_LOGI(TAG, "[THIRD] Value acceptable: avg=%d, diff1=%d, diff2=%d, checking stability...",
                                 average, diff_from_first, diff_from_second);
                        
                        // Check if value is stable
                        if (s_calibration_last_average == 0) {
                            // First reading, initialize
                            ESP_LOGI(TAG, "[THIRD] Starting stability tracking at value=%d", average);
                            s_calibration_last_average = average;
                            s_stable_start_time = xTaskGetTickCount();
                            s_stable_value = average;
                        } else {
                            ...
                            TickType_t stable_duration = (s_stable_start_time > 0) ? 
                                                         (current_time - s_stable_start_time) : 0;
                            static uint8_t s_stability_log_counter = 0;
                            s_stability_log_counter++;
                            if (s_stability_log_counter >= 20) {
                                s_stability_log_counter = 0;
                                ESP_LOGI(TAG, "[THIRD] Stability check: avg=%d, stable_val=%d, diff=%d, stable_ms=%lu, need_ms=%d",
                                         average, s_stable_value, diff, 
                                         (unsigned long)(stable_duration * portTICK_PERIOD_MS),
                                         CALIBRATION_STABILITY_TIME_MS);
                            }
                            ...
                            if (diff < 10) {  // Value is stable (within 10 units)
                                ...
                                if (abs(average - s_stable_value) < 10) {
                                    ...
                                    // Value changed but still stable, update stable value and time
                                    ESP_LOGW(TAG, "[THIRD] Stable value drifted: avg=%d, was=%d, resetting stability timer",
                                             average, s_stable_value);
                                    s_stable_value = average;
                                    s_stable_start_time = xTaskGetTickCount();
                                }
                                ...
                                // Value changed significantly, reset stable tracking
                                ESP_LOGW(TAG, "[THIRD] Value unstable (diff=%d >= 10), resetting: avg=%d, last_avg=%d",
                                         diff, average, s_calibration_last_average);
                                s_stable_value = average;
                                s_stable_start_time = xTaskGetTickCount();
                            }
...
{
    // 线性霍尔传感器模式下不支持重新校准
    ESP_LOGW(TAG, "Recalibration not supported for linear Hall sensor");
}
#endif
