    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            if (cmd == CMD_BASE_ANGLE_CONTROL) {  // Base angle control
                ...
                // Validate value range: 0~185 maps to -95°~+95° (value=90 is 0°/Home)
                // Reject invalid values (like DOA errors that send 49151)
                if (value > 185) {
                    ESP_LOGW(TAG, "Invalid angle value %u (>185), ignoring command", value);
                    continue;
                }
                
                // Convert to target angle: value=90 means 0°, value=0 means -90°, value=185 means +95°
                int16_t target_angle = (int16_t)value - 90;
                
                // Get current angle and calculate rotation needed
                float current_angle = stepper_get_current_angle();
                float rotation = target_angle - current_angle;
                
                ESP_LOGI(TAG, "Angle control: value=%u, target=%d°, current=%.1f°, rotation=%.1f°", 
                         value, target_angle, current_angle, rotation);

                // Execute angle control (absolute positioning based on current angle tracking)
                if (rotation != 0) {
                    stepper_rotate_angle_with_accel(rotation, STEPPER_SPEED_FAST);  // Use FAST speed to prevent step loss
                ...
                switch (action) {
                    case STEPPER_ACTION_SHAKE_HEAD:
                        ...
                        stepper_shake_head(50.0, 2, 800);  // 幅度50°, 速度800μs (与DECAY一致)
                        stepper_motor_power_off();
                    ...
                    case STEPPER_ACTION_SHAKE_HEAD_DECAY:
                        ...
                        stepper_shake_head_decay(40.0, 0.75, 800);  // 幅度40°, 速度800μs
                        stepper_motor_power_off();
                        control_serial_send_action_complete();  // Send action complete notification
                        break;
                    ...
                    case STEPPER_ACTION_LOOK_AROUND:
                        ...
                        stepper_look_around(55.0, 55.0, 15.0, 600, 800, 800);  // 幅度55°, 速度统一800μs
                        stepper_motor_power_off();
                        control_serial_send_action_complete();  // Send action complete notification
                        break;
                    ...
                    case STEPPER_ACTION_BEAT_SWING:  // action = 3: drum beat
                        ...
                        stepper_beat_swing(35.0, 800);  // 幅度35°, 速度800μs
                        stepper_motor_power_off();
                        // control_serial_send_action_complete();  // Send action complete notification
                        break;
                    ...
                    case STEPPER_ACTION_CAT_NUZZLE:
                        ...
                        stepper_cat_nuzzle(35, 3, 800);  // 幅度35°, 速度800μs (原1500太慢)
                        stepper_motor_power_off();
                        control_serial_send_action_complete();  // Send action complete notification
                        break;
            ...
            else if (cmd == CMD_BASE_ANGLE_QUERY) {  // Base angle query command
                ESP_LOGI(TAG, "Angle query command received");
                // Get current angle from stepper motor module
                float current_angle = stepper_get_current_angle();
                // Send angle response
                control_serial_send_angle_response(current_angle);
            }
            else {
...
/**
 * @brief Send current base angle response
 * 
 * @param angle Current angle in degrees (-95 to +95, 0 = home position)
 * @return esp_err_t ESP_OK indicates success
 * 
 * Packet format:
 * AA 55 00 03 04 XX XX CS
 * [0-1]: Header (0xAA 0x55)
 * [2-3]: Data length (0x00 0x03 = 3 bytes)
 * [4]:   Command code (0x04 = angle response)
 * [5-6]: Angle data (big-endian, signed int16 * 10 for 0.1 degree precision)
 *        Range: -950 to +950 (representing -95.0 to +95.0 degrees)
 * [7]:   Checksum (command code + data bytes sum)
 */
esp_err_t control_serial_send_angle_response(float angle)
{
    uint8_t tx_buffer[8];
    
    // Convert angle to signed integer with 0.1 degree precision (angle * 10)
    // Range: -950 to +950 (representing -95.0 to +95.0 degrees)
    int16_t angle_value = (int16_t)(angle * 10.0f + (angle >= 0 ? 0.5f : -0.5f));
    ...
    // Command code
    tx_buffer[4] = CMD_BASE_ANGLE_RESPONSE;  // 0x04
    
    // Angle data (big-endian, high byte first) - signed int16
    tx_buffer[5] = (angle_value >> 8) & 0xFF;  // High byte
    tx_buffer[6] = angle_value & 0xFF;          // Low byte
    ...
    if (sent == sizeof(tx_buffer)) {
        ESP_LOGI(TAG, "Angle response sent: %.1f degrees (raw value: %d)", angle, angle_value);
        return ESP_OK;
    } else {
        ESP_LOGE(TAG, "Failed to send angle response: sent=%d", sent);
        return ESP_FAIL;
    }
}
